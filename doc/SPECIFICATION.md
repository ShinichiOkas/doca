...
- **`SendMessage`**: タスクの投入
  ...
  - **`params.configuration`（任意）**: 親エージェントからタスク単位の設定を渡します。
    ...
    - `workspacePaths` (str[]): **このタスクに限り、ワークスペース（実行時ディレクトリ）に加えて読み書きを許可する追加ディレクトリのリスト。** 親エージェントが指定した協働フォルダ（Coworkingフォルダ等）を doca のサンドボックス外であっても扱えるようにするための仕組みです。指定された各ディレクトリ配下のみが許可対象で、前方一致による誤許可は行いません（`os.path.commonpath` による厳密判定）。タスクごとに分離され、他タスクの許可設定には影響しません。
      - **作業ベースディレクトリ**: リストの**先頭エントリ**は、そのタスクの「作業ベースディレクトリ」として扱われます。これにより、相対パス（例: `write_file('out.py')`）の解決基準と `run_command` の作業ディレクトリ（cwd）が `WORKSPACE_DIR` ではなくこの先頭フォルダになります。結果として、エージェントが相対パスで書いた成果物も自動的に指定フォルダ配下へ格納されます。`workspacePaths` 未指定時は従来どおり `WORKSPACE_DIR` が基準です。
    - リクエスト例（協働フォルダを許可する場合）:
      ...
      ```
    - 補足: 先頭エントリを作業ベースとするため、成果物の格納先を1つの協働フォルダに固定したい場合は、そのフォルダを `workspacePaths` の先頭に指定してください。複数指定した残りのエントリは「読み書き許可」のみが付与されます（作業ベースにはなりません）。
